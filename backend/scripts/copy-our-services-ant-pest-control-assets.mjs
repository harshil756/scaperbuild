#!/usr/bin/env node
/**
 * Copy our-services-ant-pest-control page images to backend storage.
 */
...
const dataFile = path.join(__dirname, '../database/data/our-services-ant-pest-control-page.json')
const srcDir = path.join(__dirname, '../../frontend/public/assets/images')
const destDir = path.join(__dirname, '../storage/app/public/cms/our-services-ant-pest-control')

if (!fs.existsSync(dataFile)) {
  console.error('Run extract-our-services-ant-pest-control-cms.mjs first.')
  process.exit(1)
}
...
function collect(pathOrMeta) {
  ...
  if (pathOrMeta.metadata) {
    ...
    if (pathOrMeta.metadata.images) {
      for (const img of pathOrMeta.metadata.images) {
        if (img.image_src) filenames.add(path.basename(img.image_src))
      }
    }
    ...
    if (pathOrMeta.metadata.tips) {
      for (const tip of pathOrMeta.metadata.tips) {
        if (tip.image_src) filenames.add(path.basename(tip.image_src))
      }
    }
  }
}
